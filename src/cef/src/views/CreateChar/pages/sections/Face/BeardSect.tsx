...
const BeardSect = ({ dataChar, handleChange, updateFaceFeature }: ISectionHead) => {
  const sectionRef = useSmoothWheelScroll()
  const listImgRef = useSmoothWheelScroll()
  const beardsOptions = Array.from({ length: 30 }, (_, i) => i)
  ...
  return (
      <section className="section-part">
        <div className="select-img-block">
          <span className="title-img">Борода</span>
          <div className="list-imgs" ref={listImgRef}>
            { beardsOptions.map((id) => (
                <li
                    key={id}
                    className={`item-img ${id === dataChar.beard ? 'selected' : ''}`}
                    onClick={() => handleChange('beard', id)}
                >
                  <img
                      loading='lazy'
                      src={`assets/img/create-char/beards/${id}.png`}
                      onError={(e) => {
        ...
        <div className="block-color">
          <span className="header-color">Цвет бороды</span>
          <div className="list-colors" ref={sectionRef}>
            { eyebrowsColor.slice().reverse().map((color, idx) => (
                <li
                    className={`item-color ${color.id === dataChar.beardColor ? 'selected' : ''}`}
                    style={{ background: color.hex }}
                    onClick={() => handleChange('beardColor', color.id)}
                    key={idx}
                ></li>
            )) }
          </div>
