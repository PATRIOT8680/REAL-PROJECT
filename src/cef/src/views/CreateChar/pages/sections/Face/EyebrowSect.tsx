...
const EyebrowSect = ({ dataChar, handleChange, updateFaceFeature }: ISectionHead) => {
  const sectionRef = useSmoothWheelScroll()
  const listImgRef = useSmoothWheelScroll()
  const eyebrowOptions = Array.from({ length: 33 }, (_, i) => i)
  ...
  return (
      <section className="section-part">
        <div className="select-img-block">
          <span className="title-img">Брови</span>
          <div className="list-imgs" ref={listImgRef}>
            {eyebrowOptions.map((id) => (
                <li
                    key={id}
                    className={`item-img ${id === dataChar.eyebrow ? 'selected' : ''}`}
                    onClick={() => handleChange('eyebrow', id)}
                >
                  <img
                      loading='lazy'
                      src={`assets/img/create-char/eyebrows/${id}.png`}
                      onError={(e) => {
        ...
        <div className="block-color">
          <span className="header-color">Цвет бровей</span>
          <div className="list-colors" ref={sectionRef}>
            { eyebrowsColor.slice().reverse().map((color, idx) => (
                <li
                    className={`item-color ${color.id === dataChar.eyebrowColor ? 'selected' : ''}`}
                    style={{ background: color.hex }}
                    onClick={() => handleChange('eyebrowColor', color.id)}
                    key={idx}
                ></li>
            )) }
          </div>
        ...
        <Slider title='Отступ бровей' value={dataChar.faceFeatures[7]}
            onChange={(value: number) => updateFaceFeature(7, value)}
            minVal={-1} maxVal={1}
            step={0.02} enterInput={false} random={true}
            subtitleOne='Вовнутрь' subtitleTwo='Наружу'
        />

        <Slider title='Высота бровей' value={dataChar.faceFeatures[6]}
            onChange={(value: number) => updateFaceFeature(6, value)}
            minVal={-1} maxVal={1}
            step={0.02} enterInput={false} random={true}
            subtitleOne='Выше' subtitleTwo='Ниже'
        />
      </section>
  )
}

export default EyebrowSect